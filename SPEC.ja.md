...
### A.11.5b 共有ヘルパー契約 (PSA8001 で強制)

PSA8001 (ファイル間 function body drift) は、 **34 個のヘルパー関数** が 4 つのパイプラインスクリプト間で byte レベルで一致することを enforce します。 r60 / r28 / r10 / r10 時点での契約対象:
...
**ローカルで契約検証する方法**: 4 スクリプトすべてを単一の psa.py 呼出しで渡すこと。 上記 34 関数のいずれかに drift があれば、 PSA8001 が関数 header 行を指して error を emit します。 意図的にスクリプト固有な関数 (phase 関数、 `Show-Help`、 `Show-PhaseList`、 `Find-KitTool`、 各ドライバファミリ固有のヘルパー) は `.psa.config.json` の `psa8001_ignore_functions` に登録済み。 4 スクリプト中 2-3 のみで同期している関数 (e.g., AMD ファミリのみのヘルパー、 MSBthPan 固有のヘルパー) は、 4 番目のスクリプトでの不在が設計意図のため、 現時点では enforce 対象外です。

新規の共有ヘルパーを 4 スクリプト全体で同期させたい場合は、 4 つすべてに同一 body で追加し、 `psa8001_ignore_functions` には登録 **しない** こと。 以降は PSA8001 が同期不変性を能動的に enforce します。
