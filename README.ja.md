...
- **姉妹スクリプト整合性の自動強制**。 PSA8001 (psa.py 3.2.0 新規ルール) が **34 個の共有ヘルパー関数** を能動的にガードします (`Format-Elapsed`、 `Write-Detail`、 `Start-DebugTrace` / `Stop-DebugTrace` / `Write-DebugFailureReport` ファミリ全体、 `Export-DebugTraceJson`、 Secure Boot ベースライン診断 trio の全メンバー、 4 つの `Write-*` ログライン ヘルパー、 `_DebugTrace_*` プライベートヘルパー集合のすべてを含む)。 今後 4 スクリプト間でこれらに drift が生じる PR は静的解析ゲートで失敗します。
