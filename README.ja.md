...
# NPU スクリプト (実機 NPU を持つホストでのみ意味があります。EPYC/AWS では -AssumeIfMissing が必要)
...
| **2** | `-AmdAccountUser <email> -AmdAccountPassword <SecureString>` | スクリプトに EULA 受諾フローを自動実行させたい場合。**best-effort で AMD 側のフォーム変更により予告なく破綻する可能性。** |
...
```powershell
# 非 NPU ホスト (例: パイプライン回帰テスト用 AWS EPYC EC2)
.\Deploy-AMDNpuDriverOnWindowsServer.ps1 `
    -Action PrepareVerify `
    -CleanWorkRoot `
